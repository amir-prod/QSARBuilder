...
def evaluate_features(individual):
    """
    Evaluate a subset of features using RandomForest classification.
    Returns a tuple (mcc_score, accuracy) for multi-objective optimization.
    """
    # Get selected features (where individual has 1)
    selected_features = [i for i, bit in enumerate(individual) if bit == 1]
    
    # Must have exactly N_FEATURES_TO_SELECT features
    ...
    # Return MCC (maximize) and accuracy (maximize)
    return (mcc, acc)


# Create DEAP types
# We want to maximize both objectives (MCC and accuracy)
...
for gen in range(N_GENERATIONS):
    ...
    best_mcc, best_acc = best_ind.fitness.values
    
    print(f"  Best MCC: {best_mcc:.4f}, Best Accuracy: {best_acc:.4f}")
    print(f"  Avg MCC: {record['avg'][0]:.4f}, Avg Accuracy: {record['avg'][1]:.4f}")
    print()
...
print("Training final RandomForest classifier with selected features...")
